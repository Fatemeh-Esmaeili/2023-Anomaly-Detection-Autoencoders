%% Making Structures for Train semisupervised
...
% Import Signals 
signalsFolder = dir("Data\*-Train.csv");
...
signalTitleList = extractBefore(signalTitle, '-Train.csv');
...
for k=1:length(analyteConcentration)
    ...
    loopSegmentTable = cell2table(loopSegmentLabels, "VariableNames",vars);
    loopBinaryTable = cell2table(loopBinaryLabels, "VariableNames",vars);
...
for i = 1: size(signalsTables,1)
    ...
    signalTitle = extractBefore(signalTitle, '-Train.csv');
...
s(2).SegLabel = removevars(s(2).SegLabel, 'S013');
 s(2).BiSegLabel = removevars(s(2).BiSegLabel, 'S013');

% Save Structure 
filepath = "DataTrain";
structureName = strcat(filepath,'\','1AdeTrainSemiSuperised','.mat');
